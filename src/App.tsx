...
function App() {
  ...
  return (
    ...
    <div className="  p-2 h-screen items-center flex justify-center  ">
      <div className=" space-y-2 bg-[#8ecae6] p-4  rounded-2xl">
        <div className="space-x-2">
            <label htmlFor="country" className=" font-mono font-bold uppercase">Country</label>
            <select className=" font-mono p-1 rounded-lg sm:w-auto w-2/4 " onChange={findCity}  name="country" id="country">
            <option defaultChecked value='nothing'>Choose Country</option>
              {countries.map((country,key)=>
              <option value={country.iso2} key={key}>{country.country}</option>
              )}
            </select>
        </div>
        <div className={`space-x-2 sm:text-center ${show=== false ? 'hidden ' : ''}`}>
            <label htmlFor="City" className=" font-mono font-bold uppercase">City</label>
            <select className=" font-mono p-1 rounded-lg sm:w-auto w-2/4 "  name="City" id="City" onChange={findWether}>
